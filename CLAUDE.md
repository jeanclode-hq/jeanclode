...
- `issue_resolve/` — triage → fix → open PR(s); triage explores the codebase itself and hands findings straight to the fixer (no separate plan stage); triage's `code_change` flag lets the same fixer answer on the issue instead (a KPI, data, an explanation) with no branch, push gate or PR, and a fix can carry an answer comment too; supports GitHub issues and GitLab issues/work_items; when a fix needs one or more linked repos beyond the issue's own, each gets a worktree and — once the fixer actually pushes to it — its own PR, opened lazily rather than speculatively. Which repos are available to it is resolved by `related_repos` (see backend/CLAUDE.md): repo groups, the org's always-include list, and the subgroup pack
...
- `memory_curate/` — daily pass over a workspace's agent memory (ADR-009): batches of entries changed since the last pass go to a curator agent with the memory tool and nothing else, which deletes run logs and noise, merges duplicates, cuts narratives down to the lesson and moves misfiled entries. Dispatched by the backend's `memory` plugin; `delete` is a soft delete (`deleted_at`), so nothing is lost
