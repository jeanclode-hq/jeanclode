...
- This is a first, deliberately narrow implementation. Not built, intentionally: memory expiration or garbage collection, per-entry access control beyond workspace scoping, size-based eviction beyond the flat per-entry and per-workspace caps. None of it gets built unless real usage shows it's needed.
