...
async def launch_respond_container(
    *,
    ...
    ``repo`` (the mention's own repo, if known) is used only to size the
    ``/tmp`` workspace — respond clones just that one repo. A mention that
    needs a cross-repo change should route to ``jeanclode:resolve``.
