"""GitLab container watcher.

Thin wrapper around a ContainerBackend (Docker or Kubernetes) scoped to
"gitlab". All watching infrastructure (events, reconciliation, log
...
from api.plugins.gitlab.config import GitLabWatcherConfig

logger = logging.getLogger(__name__)


class GitLabWatcher:
    """GitLab-specific container watcher.

    Wraps a ContainerBackend (Docker or Kubernetes) scoped to
    plugin="gitlab" and manages the watching + reconciliation lifecycle.
    """

    def __init__(
        self,
        backend: ContainerBackend,
        config: GitLabWatcherConfig,
    ) -> None:
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "GitLab watcher started (reconcile_interval=%ds)",
    ...
    async def stop(self) -> None:
        ...
        logger.info("GitLab watcher stopped")
    ...
    async def _reconcile_loop(self) -> None:
        ...
        while True:
            try:
                ...
                logger.error(f"Error in gitlab reconciliation loop: {e}")
