"""Memory curator container watcher.

Thin wrapper around a ContainerBackend (Docker or Kubernetes) scoped to
"memory". All watching infrastructure (events, reconciliation, log
...
from api.plugins.memory.config import MemoryWatcherConfig
...
class MemoryWatcher:
    ...
    plugin="memory" and manages the watching + reconciliation lifecycle.
    """
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "Memory watcher started (reconcile_interval=%ds)",
    ...
    async def _reconcile_loop(self) -> None:
        ...
        while True:
            try:
                ...
                logger.error(f"Error in memory reconciliation loop: {e}")
