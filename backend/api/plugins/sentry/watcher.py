"""Sentry container watcher.

Thin wrapper around a ContainerBackend (Docker or Kubernetes) scoped
to "sentry". All watching infrastructure (events, reconciliation,
log streaming) lives in the container plugin — this module just
...
from api.plugins.sentry.config import SentryWatcherConfig

logger = logging.getLogger(__name__)


class SentryWatcher:
    """Sentry-specific container watcher.

    Wraps a ContainerBackend (Docker or Kubernetes) scoped to
    plugin="sentry" and manages the watching + reconciliation lifecycle.
    """
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "Sentry watcher started (reconcile_interval=%ds)",
    ...
    async def stop(self) -> None:
        ...
        logger.info("Sentry watcher stopped")
    ...
    async def _reconcile_loop(self) -> None:
        ...
        while True:
            try:
                ...
                logger.error(f"Error in sentry reconciliation loop: {e}")
