"""Reusable container launch logic for GitHub-driven CLI dispatch.

Used by both the manual review/summary trigger (`POST /pull-requests/{id}/review`)
and the webhook-driven auto-trigger (PR opened / synchronized / labeled).

Sandboxing is unconditional. Credentials are resolved from authoritative
configuration (LLM via admin DB config; git platform via the installation
token for the org owning the PR's repo) and handed to the K8s/Docker
backend as ``UpstreamCredential`` entries. The credential resolvers
themselves live in :mod:`api.plugins.container.dispatch_inputs` so this
module only owns workflow-specific command shape.
"""
...
def build_command(workflow: ExecutionWorkflow, pr: PullRequest) -> list[str]:
    """Build the agent CLI command for a PR-driven workflow.

    The CLI's GitHub adaptor matches a PR URL and picks the workflow from
    the leading subcommand:
      * REVIEW (default for github URLs) → just the PR URL.
      * SUMMARY → ``summary <pr_url>``.
      * RESPOND → ``respond <target_url>`` (target_url may be a PR or
        issue URL — built by ``build_respond_command`` which has the
        mention context).

    FIX-on-PR isn't supported yet; fall back to bare URL so the CLI errors
    sensibly rather than us silently skipping the workflow.
    """
...
async def build_dispatch_inputs(
    ...
    *,
    ...
    The PR's repo is already in the git org (no Sentry-style mapping
    walk), so the git org id is just ``pr.repository.org_id``.

    Third-party skills are filtered by ``workflow`` so a marketplace
    plugin opted into "review" doesn't leak into a "summary" run (and
    vice versa) — matches sentry's behaviour where the resolver is
    keyed on the actual workflow.
    ...
    if repository:
        await add_git_platform_to_inputs(
            inputs,
            git_org_id=repository.org_id,
            repo_token_override_encrypted=repository.auth_token_encrypted,
        )
    else:
        logger.warning("git platform creds skipped: PR %s has no repository", pr.id)
...
async def launch_container(
    ...
    *,
    ...
    """Launch a sandboxed CLI container for a PR workflow.

    One container per execution. The K8s/Docker backend always
    sandboxes — credentials never reach the agent container.
    ...
    if not github_plugin or not github_plugin._watcher:
        logger.error("GitHub watcher not available, cannot dispatch")
        if db_plugin:
            with db_plugin.session() as db:
                db_update_execution_status(
                    db,
                    ...
                    error_detail="GitHub watcher not available",
                )
        return None
    ...
    try:
        ...
        logger.info(
            "Dispatched github container %s for execution %s (workflow=%s, pr=%s, upstreams=%d)",
            container_id[:12],
            str(execution_id)[:8],
            workflow.value,
            pr.pr_url,
        ...
        logger.exception("Failed to dispatch github container, marking execution as failed")
        if db_plugin:
            with db_plugin.session() as db:
                db_update_execution_status(
                    db,
...
async def launch_issue_resolve_container(
    *,
    ...
    """Launch a sandboxed CLI container for an issue-resolve run.

    The container's only argument is the issue URL. The CLI fetches
    issue body and comments from the GitHub API itself.
    ...
    if not github_plugin or not github_plugin._watcher:
        logger.error("GitHub watcher not available, cannot dispatch issue-resolve")
        if db_plugin:
            with db_plugin.session() as db:
                db_update_execution_status(
                    db,
    ...
    try:
        ...
        logger.info(
            "Dispatched github issue-resolve container %s for execution %s (issue=%s)",
            container_id[:12],
            str(execution_id)[:8],
            issue_url,
        )
        ...
        logger.exception("Failed to dispatch github issue-resolve container")
        if db_plugin:
            with db_plugin.session() as db:
                db_update_execution_status(
                    db,
...
async def launch_respond_container(
    *,
    ...
    """Launch a sandboxed CLI container for an @jeanclode mention.

    The container's only argument is ``target_url`` — a comment URL whose
    fragment encodes the surface and comment ID. The CLI fetches body /
    ...
    if not github_plugin or not github_plugin._watcher:
        logger.error("GitHub watcher not available, cannot dispatch respond")
        if db_plugin:
            with db_plugin.session() as db:
                db_update_execution_status(
                    db,
    ...
    try:
        ...
        logger.info(
            "Dispatched github respond container %s for execution %s (target=%s)",
            container_id[:12],
            str(execution_id)[:8],
            target_url,
        )
        ...
        logger.exception("Failed to dispatch github respond container")
        if db_plugin:
            with db_plugin.session() as db:
                db_update_execution_status(
                    db,
