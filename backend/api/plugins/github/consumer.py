"""FastStream consumers for the github plugin.

Subscribers:
- ``jeanclode.events.github.manual_dispatch`` — manual + webhook-driven
  workflow trigger (REVIEW / SUMMARY / RESPOND / ISSUE_RESOLVE)
- ``jeanclode.github.execution.status`` — container watcher status updates
"""
...
class ManualDispatchEvent(BaseModel):
    ...
    REVIEW/SUMMARY target a single PR via ``pull_request_id``. RESPOND
    targets either a PR (mention on a PR / PR thread / PR review) or an
    Issue (mention on a standalone issue) and carries the comment
    ``target_url`` — the CLI fetches body / surface from the API.
    ...
    execution_id: str
    organization_id: str | None = None  # the git org (target repo's org)
    workflow: str | None = None  # "review" | "summary" | "respond"
    pull_request_id: str | None = None
    issue_id: str | None = None
    target_url: str | None = None  # set for RESPOND
...
async def _consume_respond_dispatch(
    ...
    """Branch of ``consume_manual_dispatch`` for RESPOND.

    Either ``pull_request_id`` (mention on a PR/PR thread/PR review body)
    or ``issue_id`` (mention on a standalone issue) is set. We fan out to
    ``launch_respond_container`` with whichever target we can resolve.
    """
    app = get_current_app()
    db_plugin = app.database
        ...
        if event.pull_request_id:
            ...
            if not pr:
                db_update_execution_status(
                    ...
                    error_detail=f"PR {pr_id_obj} not found",
                )
                return
            repo = pr.repository
            if repo is not None:
...
@router.subscriber(
    stream=StreamSub(
        "jeanclode.events.github.manual_dispatch",
        group="jeanclode",
        consumer="worker-1",
    )
)
async def consume_manual_dispatch(event: ManualDispatchEvent) -> None:
    ...
    REVIEW/SUMMARY paths require ``pull_request_id``. RESPOND fans out to
    a separate handler (PR or issue target).
    ...
    if event.organization_id is None:
        with db_plugin.session() as db:
            ...
            # Current PR/issue state can legitimately have changed since
            # this execution was first scheduled hours ago — re-verify
            # before falling through to the same dispatch logic a fresh
            # webhook uses.
    ...
    def _load_dispatch_target(db: Session) -> tuple[PullRequest, str] | None:
        """Load the PR and its org, expunged for use once the session is gone.
        ...
        if not pr:
            logger.error("Manual dispatch: PR %s not found", pr_id)
            db_update_execution_status(
                db,
                execution_id,
                ExecutionStatus.FAILED.value,
                error_type="pull_request_not_found",
                error_detail=f"PR {pr_id} not found",
            )
            return None
    ...
    # Only REVIEW broadcasts status to the frontend — it's the only
    # workflow with a UI status indicator. SUMMARY runs in the
    # background.
    if workflow != ExecutionWorkflow.REVIEW:
        return
...
def _resolve_execution_targets(
    ...
    Returns ``(workspace_id, pr_ids)``. ``workspace_id`` is empty when the
    execution has no linked PRs, no repository, or no parent org workspace.
    """
    with db_plugin.session() as db:
        execution = (
            db.query(Execution)
...
@router.subscriber(
    stream=StreamSub(
        "jeanclode.github.execution.status",
        group="jeanclode",
        consumer="worker-1",
    )
)
async def consume_execution_status(message: ExecutionStatusMessage) -> None:
    """Handle execution status updates from the github container watcher.
...
async def _handle_execution_status(message: ExecutionStatusMessage) -> None:
    ...
    try:
        ...
        if execution:
            ...
            # Only REVIEW broadcasts status — the UI watches review
            # progress on PR rows; SUMMARY and RESPOND run silently.
            ...
            if pr_ids:
                # One event per change, not per linked PR.
                await publish_pull_request_event(
                    workspace_id=workspace_id,
                    action="updated",
                    payload={
...
@router.subscriber(
    stream=StreamSub(
        "jeanclode.events.github.issue_resolve",
        group="jeanclode",
        consumer="worker-1",
    )
)
async def consume_issue_resolve(event: IssueResolveEvent) -> None:
    """Handle a webhook-driven issue-resolve dispatch.
